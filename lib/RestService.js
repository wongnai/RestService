...
function init({ clients, defaultHeaders = [] }) {
  if (!clients) {
...
function register({ headers = [], url, type }) {
  ...
  RestClient[type] = (req = {}) => {
    ...
    return new RequestFactory(currentType.url, { headers })
  }
}
...
module.exports = { init, reset, register, RestClient, mock }
