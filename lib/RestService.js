...
function register(ref) {
  ...
  if(interceptor) {
    if (!_.isPlainObject(interceptor) || !interceptor.success || !interceptor.error) {
  ...
  RestClient[type] = function(req) {
    ...
    return new RequestFactory(currentType.url, { headers: headers, timeout: timeout }, interceptor)
  }
}
