...
function init(ref) {
  ...
  if (
    !_.isArray(defaultHeaders) ||
    !defaultHeaders.every((element) => _.isString(element))
  ) {
    throw new Error(
      `defaultHeaders should be array, and each element should be a string!`
    );
  }
...
function register(ref) {
  ...
  let circuitBreaker = ref.circuitBreaker;
  ...
  if (interceptor) {
    if (!interceptor || !_.isPlainObject(interceptor)) {
      throw new Error(
        `interceptor is not an object or, interceptor.success or interceptor.error are undefined.`
      );
    }
  ...
  if (circuitBreaker && !_.isFunction(circuitBreaker)) {
    throw new Error(`circuit breaker is not a function`);
  }
