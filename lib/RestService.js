...
function register(ref) {
  ...
  if(circuitBreaker) {
    if(!_.every(circuitBreaker, element => _.isFunction(element))) {
      throw new Error(`circuitBreaker property is not a function.`)
    }
  }
  ...
  RestClient[type] = function(req) {
    ...
    return new RequestFactory(currentType.url, options, interceptor, circuitBreaker)
  }
}
