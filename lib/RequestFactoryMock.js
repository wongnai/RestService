...
class RequestFactoryMock {
  ...
  build() {
    if (this.urlArray.length === 0) {
      throw new Error(
        `Please add at less one action with get, post, put, patch, or delete method before build mock.`
      );
    }
  ...
  action(url, method, data) {
    if (this.state === MockState.CREATED) {
      ...
      if (!result) {
        throw new Error(
          `URL ${url}, Method ${method} is not found in RequestFactoryMock. Please specific path or use '*'`
        );
      }
