...
class RequestFactory {
  ...
  getOptions(path, method, body) {
    ...
    if (this.circuitBreaker) {
      result.circuitBreaker = this.circuitBreaker
    }
    return result
  }
...
let baseHttp = function(options) {
  return RequestAsync.request(options)
      .then(r => {
        const diff = process.hrtime(this.startTime)
        r.responseTime = (diff[0] * 1e9) + diff[1]
        return r
      })
...
let doRequest = function(options) {
  ...
  let baseFn = () => {
    return baseHttp.call(this, options);
  }
  let invokerFn
  if (options.circuitBreaker) {
    invokerFn = options.circuitBreaker(baseFn, options)
  } else invokerFn = baseHttp.call(this, options)
